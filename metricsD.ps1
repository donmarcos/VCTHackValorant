...
$outputMetricFile = "outputjsonl\testvalprocfile_with_metricsD.json"
...
# Define covariance calculation function
function CalculateCovariance {
    param ($killsPerRound)
    if ($killsPerRound.Count -le 1) { return 0 } # Not enough rounds to calculate covariance

    $mean = ($killsPerRound.Values | Measure-Object -Average).Average
    Write-Host "Covariance  mean is $mean "
    $n = $killsPerRound.Count
    $covarianceSum = 0

    foreach ($round in $killsPerRound.Keys) {
        Write-Host "diff is " $killsPerRound[$round] " - $mean "
        $diff = $killsPerRound[$round] - $mean
        $covarianceSum += $diff * $diff
    }
        ...
        "playerDied" {
            ...
            if ($null -ne $event.KillerId) {
                ...
                if (-not $gameMetrics[$gameId]["PlayerMetrics"][$killerIdStr]["KillsPerRound"].ContainsKey($event.RoundNumber)) {
                    $gameMetrics[$gameId]["PlayerMetrics"][$killerIdStr]["KillsPerRound"][$event.RoundNumber.ToString()] = 0  # Key as stri
                }
...
# Calculate and store covariance after processing all events
foreach ($gameId in $gameMetrics.Keys) {
    foreach ($playerId in $gameMetrics[$gameId]["PlayerMetrics"].Keys) {
        $killsPerRound = $gameMetrics[$gameId]["PlayerMetrics"][$playerId]["KillsPerRound"]
        Write-Host "Playerid $playerId  Kills per round $killsPerRound "
        $covarianceKillsPerRound = CalculateCovariance($killsPerRound)
        $gameMetrics[$gameId]["PlayerMetrics"][$playerId]["CovarianceKillsPerRound"] = $covarianceKillsPerRound
    }
}
